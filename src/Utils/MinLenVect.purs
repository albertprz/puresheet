...
sort :: forall len a. Ord a => Compare len (-1) GT => MinLenVect len a -> MinLenVect len a
sort = overArray Array.sort

zipGT :: forall len len' a b. Compare len (-1) GT => Compare len' (-1) GT => Compare len' len GT
       => MinLenVect len a -> MinLenVect len' b -> MinLenVect len (a /\ b)
zipGT xs ys = unsafeFromArray $ Array.zip (toArray xs) (toArray ys)

zipLT :: forall len len' a b. Compare len (-1) GT => Compare len' (-1) GT => Compare len' len LT
       => MinLenVect len a -> MinLenVect len' b -> MinLenVect len' (a /\ b)
zipLT xs ys = unsafeFromArray $ Array.zip (toArray xs) (toArray ys)

zip :: forall len a b. Compare len (-1) GT
       => MinLenVect len a -> MinLenVect len b -> MinLenVect len (a /\ b)
...
overArray :: forall len a b. Compare len (-1) GT => (Array a -> Array b) -> MinLenVect len a -> MinLenVect len b
